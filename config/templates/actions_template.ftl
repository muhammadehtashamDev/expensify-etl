...
<#if addHeader == true>"report_id","report_name","action_data"
</#if><#list reports as report><#list (report.actionList![]) as act><#assign d><@toJson act/></#assign>"${report.reportID}","${(report.reportName!"")?replace('"','""')}","${d?replace('"','""')}"
</#list></#list>
